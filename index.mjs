...
(async () => {
  const tileSize = isNaN(process.env.TILE_SIZE)
    ? 256
  ...
  if (extend) {
    console.log(
      `Padding source image from ${metadata.width}x${metadata.height} to ${fullSize}x${fullSize}`
    );
    inputImage = sharp(
      await inputImage
        .resize({
  ...
  const maxZoom = Math.min(
    isNaN(process.env.MAX_ZOOM) ? zoomLevels : parseInt(process.env.MAX_ZOOM),
    zoomLevels - 1
  );
